...
class Institute_Current_InboxNoticeAdmin : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_institute_current_notice)
...
//        Toast.makeText(this@Institute_Current_InboxNoticeAdmin, begining.toString() + "  and   " + end.toString(), Toast.LENGTH_SHORT).show()
        if (InternetConnection.checkConnection(this)) { progressBar.visibility = View.VISIBLE
        try {
            mServices.GetNotice(begining.toString(), end.toString())
                .enqueue(object : Callback<APIResponse> {
                    override fun onFailure(call: Call<APIResponse>, t: Throwable) {

                        Toast.makeText(this@Institute_Current_InboxNoticeAdmin, t.message, Toast.LENGTH_SHORT).show()
...
//                                        ???
                            ...
                            if(users.isEmpty())
                            {
                                GenericUserFunction.showOopsError(
                                    this@Institute_Current_InboxNoticeAdmin,
                                    "No Notices found for the current request"
                                )
                            }else {
                                val adapter = NoticeAdapterCurrent(users, this@Institute_Current_InboxNoticeAdmin)
                            ...
                            if (result.Status.equals("No data found", ignoreCase = true)) {
                                ...
                                GenericPublicVariable.CustDialog = Dialog(this@Institute_Current_InboxNoticeAdmin)
